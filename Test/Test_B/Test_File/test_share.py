...
class TestShare(MyTest, Login, New, Delete, Share):
    def test_TestShare_01(self):
        """分享单个文件，并且验证权限是否正确(未登录状态，查看权限，设置密码，允许下载)"""
    ...
    def test_TestShare_02(self):
        """分享单个文件，并且验证权限是否正确(未登录状态，编辑权限，不设置密码，允许下载)"""
        self.login(account['username'], account['password'])  # 登录
        self.wait_element(self.my_file_loc)
        self.element_click(self.my_file_loc)  # 点击我的文件
        self.new_folder('清空专用')
