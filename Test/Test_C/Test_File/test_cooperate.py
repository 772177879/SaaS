...
class TestCooperate(MyTest, Login, New, Delete, Cooperate, Notice):
    def test_Person_TestCooperate_01(self):
        ...
        self.open_cooperate('pg协作文件', account2['username'], '查看')  # 开启协作，查看权限
        time.sleep(2)
        ...
        self.wait_element(self.my_file_loc)
        self.into_person_invite()      # 进入消息中的邀请消息中
        text = self.person_invite()      # 获取邀请消息里的内容
        invite_message = "邀请您加入协作【pg协作文件.pptx】"
    ...
    def test_Person_TestCooperate_02(self):
        ...
        self.delete_all()  # 清空文件
        self.new_file('pg', 'pg协作文件', '协作(查看权限)')  # 新建文件
        self.open_cooperate('pg协作文件', account2['username'], '查看', forbid_download=1)  # 开启协作，查看权限，但是不允许下载
        time.sleep(2)
        self.element_click(self.close_loc)  # 关闭协作弹出框
        time.sleep(2)
        self.logout()  # 退出登录
    ...
    def test_Person_TestCooperate_03(self):
        ...
        self.open_cooperate('wp协作文件', account2['username'], '编辑', forbid_download=1)  # 开启协作，编辑权限，
        # 但是不允许下载，允许添加协作人
        time.sleep(2)
        self.element_click(self.close_loc)  # 关闭协作弹出框
        time.sleep(2)
        ...
        self.into_person_invite()  # 进入消息中的邀请消息中
        text = self.person_invite()  # 获取邀请消息里的内容
        invite_message = "邀请您加入协作【wp协作文件.docx】"
        self.assertEqual(invite_message, text)
        self.close_notice()  # 关闭界面
        self.element_click(self.share_file_loc)  # 点击共享文件
        time.sleep(1)
    ...
    def test_Person_TestCooperate_04(self):
        ...
        self.delete_all()  # 清空文件
        self.new_file('wp', 'wp协作文件', '协作(编辑权限)')  # 新建文件
        self.open_cooperate('wp协作文件', account2['username'], '编辑', forbid_download=1, forbid_modify=1)  # 开启协作，
